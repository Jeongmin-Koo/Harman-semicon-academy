...
module SPI_Slave_Intf (
    ...
    reg sclk_sync0, sclk_sync1;  // ff만들기
    ...
    wire sclk_rising = sclk_sync0 & ~sclk_sync1;  // sclk_rising and gate
    wire sclk_falling = ~sclk_sync0 & sclk_sync1;  // sclk_falling and gate
    ...
    always @(*) begin
        ...
        case (so_state)
            ...
            SO_PHASE: begin  // falling edge에서 동작
                if (!SS) begin
...
module SPI_Slave_Reg (
    ...
    // reg[7:0] slv_reg[0:3]; << 이런 방법으로도 가능
    reg [1:0] state, state_next;
    reg [1:0] addr_next, addr_reg;  // addr 4개 = 2bit
    ...
    always @(*) begin
        ...
        case (state)
            ...
            ADDR_PHASE: begin
                if (!ss_n) begin
                    if (si_done) begin
                        addr_next = si_data[1:0]; // done이 들어오면 data를 받음. 하위 2비트는 주소
                        if (si_data[7]) begin
                            state_next = WRITE_PHASE;
                        end else begin
                            state_next = READ_PHASE;
                            so_start_next = 1'b1; // << 추가
                        end
                    end
                end else begin
                    state_next = IDLE;
