...
class generator;
    ...
    task run(int repeat_count);
        tx_tr = new();
        tx_tr.PADDR   = 5'h10; // UCR 레지스터 주소
        tx_tr.PWDATA  = 32'b11; // tx_enable on
        tx_tr.PWRITE  = 1;
        tx_tr.PENABLE = 1;
        tx_tr.PSEL    = 1;
...
class monitor;
    ...
    task uart_bit_monitor();
        localparam real CLK_FREQ   = 100_000_000;
        localparam real BAUD_RATE  = 115200;
        localparam int  BIT_CYCLES = int'(CLK_FREQ / BAUD_RATE); // = 868(근사치)
        ...
        forever begin
            ...
            for (int i = 0; i < 8; i++) begin
                repeat (BIT_CYCLES) @(posedge tx_intf.PCLK); // 868 count wait
                tx_tr = new();
                tx_tr.tx = tx_intf.tx;
            ...
            case (state)
                IDLE: begin
                    ...
                    if (tx_tr.PWRITE && tx_tr.PSEL && tx_tr.PENABLE && tx_tr.PADDR[4:2] == 3'd1) begin
                        ref_queue.push_back(tx_tr.PWDATA[7:0]); // 하위 8bit을 queue에 저장
                ...
                COLLECTING: begin
                    ...
                    if (bit_count == 8) begin
                        byte expected = ref_queue.pop_front(); // expected value pop from queue
                        byte actual   = rx_shift; // actual value
                        expected_data.push_back(expected); // expected value push
                        actual_data.push_back(actual); // actual value push
...
module tb_uart;
    ...
    // logic start_trigger, tx_busy, tx_done, wr_en, full, empty; // debugging code
